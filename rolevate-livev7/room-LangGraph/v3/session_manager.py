...
class SessionMetadata:
    ...
    application_details: Optional[Dict[str, Any]] = None  # Store full application details
...
class InterviewSessionManager:
    ...
    def create_session_id(self, room_id: Optional[str] = None) -> str:
        ...
        if room_id:
            # Check if room_id already starts with "interview-" to avoid duplication
            if room_id.startswith("interview-"):
                return f"{room_id}-{base_id}"
            else:
    ...
    def create_session_metadata(self, 
                              ...
                              participant_name: Optional[str] = None,
                              room_id: Optional[str] = None,
                              application_details: Optional[Dict[str, Any]] = None) -> SessionMetadata:
    ...
    async def get_or_create_session(self, 
                                  ...
                                  user_id: Optional[str] = None,
                                  participant_name: Optional[str] = None,
                                  application_id: Optional[str] = None) -> tuple[str, SessionMetadata]:
        """Get existing session or create new one with application details"""
        ...
        if application_id:
            try:
                ...
                async with GraphQLClient() as client:
                    application_details = await client.get_application_details(application_id)
                    if application_details:
                        # Update participant_name from application if not provided
                        if not participant_name:
                            participant_name = application_details.get('applicantName')
                        print(f"✅ Loaded application details for {participant_name}")
                    else:
                        print(f"⚠️ Could not load application details for ID: {application_id}")
            except Exception as e:
